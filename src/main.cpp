...
void save_file(const std::string file, float buf[], const size_t& buf_len) {
  std::ofstream f(file);
  if (!f.is_open())
    std::cerr << "Failed to open trained data output file." << std::endl;
...
int main(int argc, char* argv[])  {
  ...
  size_t weights_data_len =
      img_len + img_len * img_len * layers_len + img_len * outputs_len;
  ...
  try {
    ...
    std::cout << "Loaded trained data from file" << std::endl;
  } catch (MyException& e) {
    /* For debugging only */
  ...
  std::cout << "Img len: " << std::to_string(img_len) << ", weights len: " <<
            std::to_string(weights_data_len) << ", biases len: " <<
  ...
  for (int i = 0; i < outputs_len; i++) {
    std::cout << std::to_string(i) << ": " << std::to_string(results[i]);
    if (i + 1 < outputs_len)
      std::cout << ", ";
  }
  std::cout << std::endl;
