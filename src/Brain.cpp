...
Brain::Brain(const size_t& layers_len,
  ...
  for (int i = 0; i < m_layers_len; i++) {
    /* We need to move the trained data "input_len" elements forward to skip
       the first layers weights_data and biases_data section and then for every
       other layer we move it "input_len * input_len * i", since each layer has
  ...
  m_results.setTrainedData(&weights_data[data_idx], &biases_data[data_idx]);
...
std::unique_ptr<float[]> Brain::think(void) {
  /* Run all layers excluding the last one, sending each ones output to the
     next layer */
...
void Brain::learn(float answers[]) {
  ...
  for (int i = m_layers_len - 1; i >= 0; i--) {
    INeuronDataReceiver *l = i > 0 ?
        static_cast<INeuronDataReceiver *>(&m_layers[i - 1]) : nullptr;
