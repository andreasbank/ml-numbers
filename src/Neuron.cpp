...
void Neuron::getTrainedData(float weights[],
  ...
  if (weights_len < m_weights_len)
    throw MyException("Buffer not large enough to return trained data");
...
void Neuron::learn(INeuronDataReceiver *receiver, const float& answer) {
  ...
  /* We do not take bigger corrections than what we would have taken with 1000
     train samples. This limits the chance to step over a local minimum. */
  ...
  for (int i = 0; i < m_inputs_len; i++) {
    ...
    /* Calculate the amount of error belonging to each weight of this layer */
    weights_errors[i] =
        (m_weights[i] / weights_sum) * error;

    /* Calculate how far away from a local minimum we are, in order to take a
       smaller correction step and hopefully not overshoot past the local
       minimum */
    float gardient = (2 / train_samples) * m_inputs[i] * weights_errors[i];
